...
class VendorDB():
	...
	# Query only a single vendor based on the Vendor Code Identifier
	def selectVendor(self, vendor_code):
		...
		cursor.execute(sql_onevendor, vendor_code)
		for row in cursor:
	...
	# Query all the vendors (actually, just a sampling for this example code)
	def selectVendors(self):
		# Select a sampling of vendors
		sql_allvendors = """
		SELECT "VENDOR" vndr_cd,
		"VENDNAME1" vndr_nm,
		"ATTN" attn_txt,
